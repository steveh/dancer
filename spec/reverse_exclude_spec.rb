...
RSpec.describe "Reverse" do
  ...
  let(:end_at)   { Time.utc(2016, 8, 1, 8, 0, 0) }
  ...
  shared_examples "four minute dancer" do
    ...
    it "returns correct ranges" do
      expect(ranges[0]).to eq(Time.utc(2016, 8, 1, 8, 2,  0)...Time.utc(2016, 8, 1, 8, 1, 30))
      expect(ranges[1]).to eq(Time.utc(2016, 8, 1, 8, 1, 30)...Time.utc(2016, 8, 1, 8, 1,  0))
      expect(ranges[2]).to eq(Time.utc(2016, 8, 1, 8, 1,  0)...Time.utc(2016, 8, 1, 8, 0, 30))
      expect(ranges[3]).to eq(Time.utc(2016, 8, 1, 8, 0, 30)...Time.utc(2016, 8, 1, 8, 0,  0))
    end
